...
(function () {
  ...
  function bindMapEvents(scope, eventsStr, googleObject, element, listenonce) {
    angular.forEach(eventsStr.split(' '), function (eventName) {

      //Prefix all googlemap events with 'map-', so eg 'click'
      //for the googlemap doesn't interfere with a normal 'click' event
      
      var triggerEvent = function(element, $event, evt) {
      ...
      if(listenonce.indexOf(eventName) == -1) {
        google.maps.event.addListener(googleObject, eventName, function (evt) {
          triggerEvent(element, $event, evt);
        });
      } else {
    ...
    ['ui.config', '$parse', function (uiConfig, $parse) {
      ...
      return {
        ...
        link: function (scope, elm, attrs) {
          ...
          var listenonce = attrs.listenonce ? attrs.listenonce.split(',') : []; 

          //Set scope variable for the map
          model.assign(scope, map);

          bindMapEvents(scope, mapEvents, map, elm, listenonce);
        }
      };
    }]);
